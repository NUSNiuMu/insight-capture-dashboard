...
SYNC_CLI="${ROOT_DIR}/scripts/sync_camera_restart.py"
...
configured_dds_type() {
    ...
    python3 - "${ROOT_DIR}/config/cameras.json" 2>/dev/null <<'PY' || true
import json, sys
try:
...
expected_camera_count() {
    python3 - "${ROOT_DIR}/config/cameras.json" 2>/dev/null <<'PY' || echo 0
import json, sys
try:
