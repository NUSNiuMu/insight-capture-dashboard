...
STATE_DIM = 20
VIDEO_KEY_BY_ROLE = {
    "right_hand": "observation.images.right_hand_up",
    "left_hand": "observation.images.left_hand_up",
    "head": "observation.images.head_main",
}
HAND_OFFSET = {"right_hand": 0, "left_hand": 10}
...
def _episode_state(
    ...
    for robot_index, spec in enumerate(hand_specs):
        offset = HAND_OFFSET[spec.role]
        positions = plan.lowdim[f"robot{robot_index}_eef_pos"]
        rotvecs = plan.lowdim[f"robot{robot_index}_eef_rot_axis_angle"]
        widths = plan.lowdim[f"robot{robot_index}_gripper_width"]
        rotation_6d = Rotation.from_rotvec(rotvecs).as_matrix()[:, :2, :].reshape(-1, 6)
        state[:, offset : offset + 10] = np.concatenate(
            (positions, rotation_6d.astype(np.float32), widths), axis=1
        )
        state_valid[:, offset : offset + 10] = True
    action = np.concatenate((state[1:], state[-1:]), axis=0)
    action_valid = np.concatenate((state_valid[1:], state_valid[-1:]), axis=0)
...
def _append_episode_videos(
    ...
    *,
    ...
    while reader.has_next():
        ...
        writers[VIDEO_KEY_BY_ROLE[spec.role]].write(frame)
        written[spec.name] += 1
    missing = {
...
def _modality(specs: list[umi.CameraSpec]) -> dict[str, object]:
    state = {}
    action = {}
    for role, offset in HAND_OFFSET.items():
        side = HAND_LABEL[role]
        blocks = {
            f"{side}_end_effector_position": (offset, offset + 3, "m"),
            f"{side}_end_effector_rotation_6d": (offset + 3, offset + 9, "rotation_6d"),
            f"{side}_gripper_width": (offset + 9, offset + 10, "m"),
        }
    ...
    return {
        ...
        "action": action,
        "video": {
            VIDEO_KEY_BY_ROLE[spec.role].removeprefix("observation.images."): {
                "original_key": VIDEO_KEY_BY_ROLE[spec.role],
...
def export_lerobot_dataset(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="lerobot_export_", dir=str(output_path.parent)) as temporary:
        ...
        video_paths = {
            VIDEO_KEY_BY_ROLE[spec.role]: root
            / "videos"
            / VIDEO_KEY_BY_ROLE[spec.role]
            / "chunk-000"
            / "file-000.mp4"
            for spec in specs
        }
        writers = {
            ...
            for spec in specs
            for key, path in [(VIDEO_KEY_BY_ROLE[spec.role], video_paths[VIDEO_KEY_BY_ROLE[spec.role]])]
        }
        try:
        ...
        for spec in specs:
            features[VIDEO_KEY_BY_ROLE[spec.role]] = _video_feature(output_shapes[spec.name], fps)
        info = {
            ...
            "features": features,
            "state_layout": "[right_10d, left_10d], each xyz + rotation_6d(first two matrix rows) + gripper_width_m",
            "action_layout": "absolute next-state target; final frame repeats the final state",
            "video_processing": {
            ...
            "source": {
                "format_profile": "hifi_umi_lerobot_v3",
                "bags": [path.name for path in bag_paths],
                "camera_mapping": {VIDEO_KEY_BY_ROLE[spec.role]: spec.name for spec in specs},
        ...
        summary = {
            ...
            "missing_hand_policy": "zero_fill_with_per_dimension_validity_mask",
