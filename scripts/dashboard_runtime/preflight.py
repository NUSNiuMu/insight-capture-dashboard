...
from post_processing_core.config import probe_recording_root
...
class CapturePreflight:
    ...
    def evaluate(self, topics: Optional[Sequence[str]] = None, *, refresh_topics: bool = True) -> Dict:
        ...
        for camera in self.node.cameras:
            ...
            recent = [stamp for stamp in samples if now - stamp <= 2.0]
            fps = 0.0
            if len(recent) > 1:
                fps = (len(recent) - 1) / max(recent[-1] - recent[0], 1e-6)
            fresh = age is not None and age <= self.camera_stale_sec
            camera_health[camera.name] = {"fresh": fresh, "age_sec": age, "fps": round(fps, 2)}
            if not fresh:
                failures.append(self._failure(
                    "camera_stale", f"{camera.label}无新鲜图像数据", camera=camera.name, age_sec=age
                ))
