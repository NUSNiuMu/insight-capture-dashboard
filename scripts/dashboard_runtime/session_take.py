...
class SessionTakeStore:
    ...
    def __init__(self, results_root: Path, config: Optional[Dict] = None) -> None:
        ...
        self.task = str(settings.get("task") or "unspecified")
        self.operator = str(settings.get("operator") or "unknown")
        self.session_id = _slug(settings.get("session_id"), f"{today}-default")
        self.station_check_after_take = bool(
            settings.get("station_check_after_take", False)
        )
