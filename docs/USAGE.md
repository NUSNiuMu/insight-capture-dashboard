...
| 路径 | 用途 |
|---|---|
| `/` 或 `/3d` | 三路相机实时画面 + 3D VIO 轨迹（同一页，2026-07 起合并）+ 在线标定按钮 |
...
> 进入 `/3d` 后会先显示轻量姿态标记，随后依次接通三路画面、轨迹和模型；这样从其他页面切回时可以先操作场景，不必等待大型模型加载完成。
...
操作入口在 `/3d` 页的 "Alignment control" 面板——标定进行时全程盯
