...
若当前头部相机发布 HandEngine 结果，也可以在头部画面内同时做双手点赞：两只手均保持“拇指向上、其余
...
当配置的左右手相机首次都完成全局定位时，音响会再播报一次“校准完成”。
叠杯等重复任务在每条录制停止后应将三台相机放回固定治具并说“检查相机”；系统先播报
“开始检测”，再逐台比较全局 Pose 与保存值，按手部重定位严格、头部建图宽松的阈值进行
PASS/复检/重校准分级，并把结果关联到最近一条 bag。首次部署治具时由
标定负责人在确认校准正确后说“设置检测位”。详见
