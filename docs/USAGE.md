...
需要明确授权自动修复相机时钟和采集相位时运行：

```bash
./scripts/system_doctor.sh --repair
```

`--repare` 作为常见拼写错误的兼容别名，行为与 `--repair` 完全相同。修复模式会先完成一轮
只读诊断，确认未在录制后执行三台相机的 `ntpdate -b`、同步重启采集服务和 10 秒图像时间戳
测量，然后再次运行完整诊断。最终报告同时保留修复前结果、NTP 校时前后 offset、采集服务
定时器结果和图像最大差；即使 NTP 已恢复到 1 ms 内，只要图像相位仍超过 10 ms，最终结论
仍为故障并返回退出码 `2`。该参数不会自动修复录制盘、容器、网络或定位问题，这些项目仍只
输出证据和人工修复建议；录制中会拒绝任何校时或服务重启。采集服务恢复期间 Dashboard
看门狗可能重建后端 DDS participant，这是修复流程的预期影响，不会重启整台相机。
