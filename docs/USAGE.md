...
1. 在 `/` 的 Sessions 页面新建或选择 Task Set，然后点 **Enter task set**；任务可直接在页面
   编辑或结束，不需要修改配置和重启。内置“叠杯子”也可通过语音进入；Task Set 不随日期或
   服务重启重建，重新进入后继续原有 Take 编号；
2. 说“系统状态”，确认三相机、定位、存储和必要录制数据流通过 Preflight；
3. 说“开始录制”；系统在当前 Task Set 中分配递增的 Take，并把 raw bag 写入当前存储根目录的
   `<task_id>/` 子目录；如果没有进入任何任务，录制会被拒绝，不会产生非任务 bag；
4. 采集完成后说“停止录制”，等待 quick QC 播报；操作失败时说“本条作废”，只标记
   invalid，不删除原始 MCAP；
5. 随时说“当前任务多少条”，系统会播报任务集累计已录制、有效、作废和下一条编号；说
   “结束当前任务”只退出当前任务集，再次进入不会清零；
...
完成后仍说“停止录制”。
校准录制也可以在 `/bags` 回放：系统固定使用 Insight3 A/B 的 `infra1/image_raw`
左目画面，并使用各自的原生 `vio_100hz` 轨迹；右目原图仍完整保留在 MCAP 中，但不进入
回放合成画面。
...
现场不再提供“设置检测位”入口；未预置检测基准的任务不启用该项检查。详见
