...
2026-09-09。网页已默认采用 Babylon camera-facing 粗线。`experiment/qt-native-dashboard` 分支新增 [Qt Quick + GStreamer + Qt Quick 3D 原型](../native_dashboard/README.md)，覆盖采集主界面的视频、轨迹、模型及控制。原型独立运行，尚未作为默认客户前端部署；下文保留选型依据。
...
| Qt Quick + Qt Quick 3D | qmlglsink（Qt5）或 qml6glsink（Qt6）共享 GL 纹理 | 完整原生视频、3D 模型/轨迹和交互 | 原型使用 QWindow + GstVideoOverlay 原生子窗口；共享纹理的 qml6glsink 路径仍需 GStreamer 1.22 起支持 |
...
NVIDIA R36.4.3 文档提供 `nvv4l2decoder ! nv3dsink` 等硬件显示管线：[Accelerated GStreamer](https://docs.nvidia.com/jetson/archives/r36.4.3/DeveloperGuide/SD/Multimedia/AcceleratedGstreamer.html)。前一轮 30 帧测试图通过 NVDEC + fakesink，证明原生硬解可用；当前原型已验证三路 Qt 原生窗口显示；尚未证明整条链路零拷贝或稳定 30 FPS。

## 3D 与性能边界

- Qt Quick 3D 的 RuntimeLoader 支持 glTF/GLB，可以尝试复用头盔和夹爪模型；轨迹可用动态 Geometry。原型已迁移姿态坐标、模型层级、夹爪开合、轨迹代次以及录制/回放控制；复杂场景仍需进一步验证。Babylon JS 代码不能直接作为 Qt Quick 3D 场景运行。[RuntimeLoader](https://doc.qt.io/qt-6/qml-qtquick3d-assetutils-runtimeloader.html)、[QQuick3DGeometry](https://doc.qt.io/qt-6/qquick3dgeometry.html)。
