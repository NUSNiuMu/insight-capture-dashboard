...
| 语音控制无响应 | 宿主机离线语音服务未运行、升级后仍残留旧 unit，或 USB 声卡未枚举 | 检查 `systemctl --user status insight-voice-control.service`、确认 `looper-openclaw-voice.service` 已迁移、再检查 `arecord -L` 和新服务日志 |
