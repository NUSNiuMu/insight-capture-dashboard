...
- 2026-08-04 在 2560×1440 X11 桌面完成双 Firefox A/B：相机墙与 3D 使用
  独立 profile、content/RDD/WebRender 进程，无边框窗口可稳定拼接，导航隐藏和
  相机全屏切换也能正确复位；但相机窗口聚焦后的 presented 仅约
  14.1/11.9/14.2 FPS，未优于此前单窗口的 17/18/21 FPS。说明主要限制仍在两个
  Firefox 共享的 GPU/桌面合成链路，不应默认启用双窗口；保留
  `INSIGHT_KIOSK_SPLIT=1` 仅供后续 A/B。
