...
- 语音“开始校准”成功 reset 后，后台监控必须先观察到本轮未完成状态，再以 Insight3 A、B
  ...
  “我在”、命令回复和异步“校准完成”争用同一个 E3 playback device。
- 固定检测位质量门比较三路相机的三组两两相对位姿，不以各自世界坐标绝对值判定，避免
  Insight9 公共回环校正产生误报。检测必须先满足定位在线、pose 新鲜和稳定窗口；每次结果
  写到 `outputs/results/capture_checks/` 并关联最近一条 bag。检测位基准跨重校准保留，只有
  已确认校准正确且治具/安装发生有效变更时才能显式覆盖。
