...
“开始校准”调用 `/api/mapping/reset`，语义是清空头部地图和手部相机全局定位状态，
立即开始新一轮在线校准，不是夹爪尺寸标定。语音桥随后在后台检查本机 mapping 状态；
配置的左右手相机在本次 reset 后首次同时进入 `localized=true` 时，播放一次预生成的
“校准完成”。

每个采集单元停止后，确认语音会要求把三台相机放回固定检测位。静止至少 1.2 秒后说
“检查相机”，系统先播报“开始检测”，再把左右手和头部当前全局 Pose 分别
与保存值比较并播报通过、重新放置或重新校准。手部重定位使用严格阈值，头部建图使用较宽
阈值；两路手部相机必须已完成本轮全局定位，但不要求检测瞬间再次命中地图。
“设置检测位”会覆盖当前基准，只能在校准已人工确认正确时使用。完整现场流程见
