...
实时预览优先显示新画面。接收端 jitter buffer 限制为 50 ms，解码后只保留一帧待显示图像；拥塞时丢弃解码后的旧帧，不直接丢弃任意 H.264 参考帧。显示端启用时钟同步、QoS 和 50 ms 最大迟到容限，并响应管线的 LATENCY 消息。

仅限制队列长度不能排除上游送来的旧帧。实时 sink 入口另外按 segment 映射后的 PTS 检查接收端播放时间，超过 150 ms 的帧直接丢弃，等待新帧。该阈值不是相机到屏幕的端到端延迟上限，也不覆盖显示驱动内部的等待；录制链路和 MP4 回放不使用这项检查。

`--diagnostics <path>` 增加以下字段：

- `latest_frame_drops`：解码后单帧队列的覆盖计数。
- `expired_frame_drops`：sink 入口因超过 150 ms 丢弃的帧数。
- `sink_pts_age_ms`：最近一帧进入 sink 前相对接收端播放时钟的年龄，负数表示 PTS 尚在未来；尚未取得样本时为 -1。它不是屏幕呈现时刻。
- `bounded_jitter_buffers`：已应用接收缓冲限制的 jitter buffer 数量。

2026-09-17 隔离验证：实际 C++ 回调在 700 ms 积压测试中，30 FPS 丢弃全部 60 个过期帧，10 FPS 丢弃全部 20 个过期帧，新帧恢复正常；原先 `sync=false/qos=false` 的对照播放了全部 60 个旧帧。真实三路视频运行 55 秒均有持续输出、generation 保持 1，实际触发了过期帧丢弃。现有含 B 帧 MP4 的暂停、跳到 5 秒和继续播放通过，回放过期帧计数为 0，正常 DPB 保留。测试位于 `~/workspaces/insight_capture_tests/native_latency_20260917/`。当前为独立分支 `experiment/qt-live-frame-deadlines`，尚未替换现场常驻 Qt；长时间运行的屏幕延迟仍需动态画面对照验收。
